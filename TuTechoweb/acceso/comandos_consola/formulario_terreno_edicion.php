...
if ($_SERVER['REQUEST_METHOD'] == 'POST') { //si se envio datos por POST guardar los datos en variables que seran temporales
  ...
  $animales_domesticos = $_POST['animales_domesticos'];
	...
	$muralla = $_POST['muralla'];
	$porton = $_POST['porton'];
	$constructible = $_POST['constructible'];
	$altura_max = $_POST['altura_max'];
	$altura_max_medida = $_POST['altura_max_medida'];
	$agua = $_POST['agua'];
	$electricidad = $_POST['electricidad'];
	$zona_franca = $_POST['zona_franca'];
  ...
  $area_verde = $_POST['area_verde'];
	...
	if (isset($_POST['guardar_borrador'])) {
		...
		if (empty($tipo_via) or empty($aceras) or empty($cobertura) or empty($descripcion_bien) or empty($tipo_zona) or empty($mapa_coordenada_lat) or empty($mapa_coordenada_lng)) { //si hay algun campo obligatorio vacio mostrar un error
    ...
    if (isset($_POST['guardar_borrador'])) {//se pareto en guardar borrado
      ...
      if ($modo_received == 'first_entry') {//es la primera vez en el formulario o es un borrador y esta en la tabla borradores
            ...
            $statement_registrar_mas = $conexion->prepare(
      				"UPDATE $tabla_to_update SET tipo_bien = :tipo_bien, precio = :precio, mantenimiento = :mantenimiento, base_imponible = :base_imponible, estado = :estado, exclusivo = :exclusivo, pre_venta = :pre_venta, anticretico = :anticretico, pais = :pais, ciudad = :ciudad, departamento = :departamento, barrio = :barrio, location_tag = :location_tag, direccion = :direccion, direccion_complemento = :direccion_complemento, superficie_terreno = :superficie_terreno, superficie_terreno_medida = :superficie_terreno_medida, llave = :llave, comision_factor = :comision_factor WHERE referencia = :referencia");
	...
	$statement1 = $conexion->prepare(
		"UPDATE $tabla_to_update SET tipo_via = :tipo_via, aceras = :aceras, residencia = :residencia, muralla = :muralla, tipo_zona = :tipo_zona, constructible = :constructible, electricidad = :electricidad, agua = :agua, geografia = :geografia, altura_max = :altura_max, altura_max_medida = :altura_max_medida, zona_franca = :zona_franca, animales_domesticos = :animales_domesticos WHERE referencia = :referencia"
	 );
    ...
    ':tipo_via' => $tipo_via,
		...
		':altura_max_medida' => $altura_max_medida,
		':zona_franca' => $zona_franca,
    ':animales_domesticos' => $animales_domesticos
	));

	$statement2 = $conexion->prepare(
		"UPDATE $tabla_to_update SET gaz_domiciliario = :gaz_domiciliario, cobertura = :cobertura, porton = :porton WHERE referencia = :referencia");
	...
	$statement3 = $conexion->prepare(
		"UPDATE $tabla_to_update SET ruido_externo = :ruido_externo, alcantarillado = :alcantarillado, desague = :desague, descripcion_bien = :descripcion_bien, parada_bus = :parada_bus, teleferico = :teleferico, supermercado = :supermercado, farmacia = :farmacia, guarderia = :guarderia, mapa_coordenada_lat = :mapa_coordenada_lat, mapa_coordenada_lng = :mapa_coordenada_lng, mapa_zoom = :mapa_zoom, escuela = :escuela, policia = :policia, hospital = :hospital, area_verde = :area_verde, comentarios_bien = :comentarios_bien WHERE referencia = :referencia");
...
require 'formulario_terreno_edicion.view.php';
?>

...
if ($_SERVER['REQUEST_METHOD'] == 'POST') { //si se envio datos por POST guardar los datos en variables que seran temporales
  ...
  $area_verde = $_POST['area_verde'];
	...
	if (isset($_POST['guardar_borrador'])) {
		...
		if (empty($tipo_via) or empty($aceras) or !isset($dormitorios) or !isset($parqueos) or !isset($piso) or empty($tipo_departamento) or empty($cocina) or !isset($wc) or
        empty($tipo_ducha) or empty($ventanas) or empty($conexion_electrica) or
				empty($cobertura) or empty($internet) or empty($tv_cable) or empty($interior_estado) or empty($descripcion_bien) or empty($jardin_estado) or empty($mapa_coordenada_lat) or empty($mapa_coordenada_lng)) { //si hay algun campo obligatorio vacio mostrar un error
    ...
    if (isset($_POST['guardar_borrador'])) {//se pareto en guardar borrado
      ...
      if ($modo_received == 'first_entry') {//es la primera vez en el formulario o es un borrador y esta en la tabla borradores
            ...
            $statement_registrar_mas = $conexion->prepare(
      				"UPDATE $tabla_to_update SET tipo_bien = :tipo_bien, precio = :precio, mantenimiento = :mantenimiento, base_imponible = :base_imponible, estado = :estado, exclusivo = :exclusivo, pre_venta = :pre_venta, anticretico = :anticretico, pais = :pais, ciudad = :ciudad, departamento = :departamento, barrio = :barrio, location_tag = :location_tag, direccion = :direccion, direccion_complemento = :direccion_complemento, superficie_inmueble = :superficie_inmueble, llave = :llave, comision_factor = :comision_factor WHERE referencia = :referencia");
	...
	$statement1 = $conexion->prepare(
		"UPDATE $tabla_to_update SET tipo_via = :tipo_via, aceras = :aceras, residencia = :residencia, dormitorios = :dormitorios, handicap = :handicap, parqueos = :parqueos, acensor = :acensor, intercomunicador = :intercomunicador, planta_baja = :planta_baja, amoblado = :amoblado, lavanderia = :lavanderia, balcon = :balcon, cocina = :cocina, equipada = :equipada, alacena = :alacena, wc = :wc, tipo_departamento = :tipo_departamento, penthouse = :penthouse, baulera = :baulera WHERE referencia = :referencia"
	 );
	...
	$statement4 = $conexion->prepare(
		"UPDATE $tabla_to_update SET piso = :piso, extintores = :extintores, escuela = :escuela, policia = :policia, hospital = :hospital, area_verde = :area_verde, comentarios_bien = :comentarios_bien, mapa_coordenada_lat = :mapa_coordenada_lat, mapa_coordenada_lng = :mapa_coordenada_lng, mapa_zoom = :mapa_zoom WHERE referencia = :referencia");
...
require 'formulario_departamento_edicion.view.php';
?>

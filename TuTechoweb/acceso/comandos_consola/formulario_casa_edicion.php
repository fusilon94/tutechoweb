...
if ($_SERVER['REQUEST_METHOD'] == 'POST') { //si se envio datos por POST guardar los datos en variables que seran temporales
  ...
  $mapa_zoom = $_POST['mapa_zoom'];
	...
	$hab_planta_baja = $_POST['hab_planta_baja'];
  ...
  $baulera = $_POST['baulera'];
	...
	$jardin = $_POST['jardin'];
	$cesped = $_POST['cesped'];
	$jardin_terraza = $_POST['jardin_terraza'];
	$jardin_superficie = $_POST['jardin_superficie'];
	$jardin_superficie_medida = $_POST['jardin_superficie_medida'];
  ...
  $animales_domesticos = $_POST['animales_domesticos'];
	$reserva_agua = $_POST['reserva_agua'];
	$reserva_agua_medida = $_POST['reserva_agua_medida'];
	$reserva_compartida = $_POST['reserva_compartida'];
  ...
  $area_verde = $_POST['area_verde'];
	...
	if (isset($_POST['guardar_borrador'])) {
		...
		if (empty($tipo_via) or empty($aceras) or !isset($dormitorios) or !isset($parqueos) or empty($pisos) or empty($cocina) or !isset($wc) or empty($tipo_ducha) or empty($ventanas) or empty($conexion_electrica) or empty($cobertura) or empty($internet)
    or empty($tv_cable) or empty($interior_estado) or empty($descripcion_bien) or empty($jardin_estado) or empty($mapa_coordenada_lat) or empty($mapa_coordenada_lng)) { //si hay algun campo obligatorio vacio mostrar un error
    ...
    if (isset($_POST['guardar_borrador'])) {//se pareto en guardar borrado
    ...
    }else {//se apreto en registrar cambios
      if ($modo_received == 'first_entry') {//es la primera vez en el formulario o es un borrador y esta en la tabla borradores
            ...
            $statement_registrar_mas = $conexion->prepare(
      				"UPDATE $tabla_to_update SET tipo_bien = :tipo_bien, precio = :precio, mantenimiento = :mantenimiento, base_imponible = :base_imponible, estado = :estado, exclusivo = :exclusivo, pre_venta = :pre_venta, anticretico = :anticretico, pais = :pais, ciudad = :ciudad, departamento = :departamento, barrio = :barrio, location_tag = :location_tag, direccion = :direccion, direccion_complemento = :direccion_complemento, superficie_inmueble = :superficie_inmueble, superficie_terreno = :superficie_terreno, superficie_terreno_medida = :superficie_terreno_medida, llave = :llave, comision_factor = :comision_factor WHERE referencia = :referencia");
      			...
      			$statement_registrar_mas->execute(array(
					...
					':superficie_terreno' => $info_bien_all['superficie_terreno'],
					':superficie_terreno_medida' => $info_bien_all['superficie_terreno_medida'],
					':llave' => $info_bien_all['llave'],
					':comision_factor' => $info_bien_all['comision_factor']
      			));
	...
	$statement1 = $conexion->prepare(
		"UPDATE $tabla_to_update SET tipo_via = :tipo_via, aceras = :aceras, residencia = :residencia, dormitorios = :dormitorios, handicap = :handicap, parqueos = :parqueos, pisos = :pisos, acensor = :acensor, intercomunicador = :intercomunicador, hab_planta_baja = :hab_planta_baja, amoblado = :amoblado, sotano = :sotano, alcoba = :alcoba, lavanderia = :lavanderia, balcon = :balcon, baulera = :baulera, cocina = :cocina, equipada = :equipada, alacena = :alacena, wc = :wc WHERE referencia = :referencia"
	 );
	...
	$statement2->execute(array(
		...
		':jardin' => $jardin,
		':cesped' => $cesped,
		':jardin_terraza' => $jardin_terraza,
		':jardin_superficie' => $jardin_superficie,
		':jardin_superficie_medida' => $jardin_superficie_medida,
	...
	$statement3 = $conexion->prepare(
		"UPDATE $tabla_to_update SET ruido_externo = :ruido_externo, picina = :picina, parrillero = :parrillero, jardin_estado = :jardin_estado, sauna = :sauna, jacuzzi = :jacuzzi, gimnasio = :gimnasio, alcantarillado = :alcantarillado, desague = :desague, animales_domesticos = :animales_domesticos, reserva_agua = :reserva_agua, reserva_agua_medida = :reserva_agua_medida, reserva_compartida = :reserva_compartida, descripcion_bien = :descripcion_bien, parada_bus = :parada_bus, teleferico = :teleferico, supermercado = :supermercado, farmacia = :farmacia, guarderia = :guarderia, mapa_coordenada_lat = :mapa_coordenada_lat, mapa_coordenada_lng = :mapa_coordenada_lng, mapa_zoom = :mapa_zoom WHERE referencia = :referencia");
    ...
    ':animales_domesticos' => $animales_domesticos,
		':reserva_agua' => $reserva_agua,
		':reserva_agua_medida' => $reserva_agua_medida,
		':reserva_compartida' => $reserva_compartida,
	...
	$statement4 = $conexion->prepare(
		"UPDATE $tabla_to_update SET escuela = :escuela, policia = :policia, hospital = :hospital, area_verde = :area_verde, comentarios_bien = :comentarios_bien WHERE referencia = :referencia");
...
require 'formulario_casa_edicion.view.php';
?>

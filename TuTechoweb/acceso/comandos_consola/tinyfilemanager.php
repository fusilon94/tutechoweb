...
if (isset($_SESSION['usuario']) && isset($entry_val)) {
    // todo of
}else {
...
$config_file = './config_tinyfilemanager.php';
...
defined('FM_ALLOWED_ITEMS') || define('FM_ALLOWED_ITEMS', $allowed_items);
...
if (is_array($objects)) {
    foreach ($objects as $file) {
        ...
        $new_path = $path . '/' . $file;
        if (@is_file($new_path) && fm_is_allowed_items($file)) {
            $files[] = $file;
        } elseif (@is_dir($new_path) && $file != '.' && $file != '..' && fm_is_allowed_items($file)) {
            $folders[] = $file;
        }
    }
}
...
?>
<form action="" method="post" class="pt-3 form_container">

    <?php

    if (isset($modo_validacion)) {
        echo"
        <div class=\"botones_container\">
        <span class=\"info_db_btn\" name=\"$tipo_file_selected\" data1=\"$pais_selected\" data2=\"$id_file\"><i class=\"fas fa-search\"></i><p>DATA BASE</p></span>
        <span class=\"final_btn_group\">
            <span class=\"reclamar_btn\">RECLAMAR</span>
            <span class=\"validar_btn\">VALIDAR</span>
        </span>
        </div>
        ";
    };
    ...
    <div class="table-responsive">
        <table class="table table-bordered table-hover table-sm <?php echo $tableTheme; ?>" id="main-table">
            ...
            foreach ($folders as $f) {
                ...
                <tr>
                    ...
                    <td class="inline-actions"><?php if (!FM_READONLY): ?>
                        ...
                        <?php endif; ?>
                        <a title="<?php echo lng('DirectLink')?>" href="<?php echo str_replace('#', '%23', fm_enc(FM_ROOT_URL . (FM_PATH != '' ? '/' . FM_PATH : '') . '/' . $f . '/')) ?>" target="_blank"><i class="fa fa-link" aria-hidden="true"></i></a>
                    </td>
                </tr>
                <?php
                flush();
            ...
            foreach ($files as $f) {
                ...
                $filelink = str_replace('#', '%23', fm_enc(FM_ROOT_URL . (FM_PATH != '' ? '/' . FM_PATH : '') . '/' . $f));
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <?php
                           if (in_array(strtolower(pathinfo($f, PATHINFO_EXTENSION)), array('gif', 'jpg', 'jpeg', 'png', 'bmp', 'ico', 'svg'))): ?>
                                ...
                                <a href="<?php echo $filelink ?>" target="_blank" data-preview-image="<?php echo $imagePreview ?>" title="<?php echo $f ?>">
                           <?php else: ?>
                                <a href="<?php echo $filelink ?>" target="_blank" title="<?php echo $f ?>">
                            <?php endif; ?>
                    ...
                    <td class="inline-actions">
                        ...
                        <?php endif; ?>
                        <a title="<?php echo lng('DirectLink') ?>" href="<?php echo str_replace('#', '%23', fm_enc(FM_ROOT_URL . (FM_PATH != '' ? '/' . FM_PATH : '') . '/' . $f)) ?>" target="_blank"><i class="fa fa-link"></i></a>
 ...
 * @return bool
 */
function fm_is_allowed_items($file) {
    $ext = strtolower(pathinfo($file, PATHINFO_EXTENSION));
    if(in_array($file, FM_ALLOWED_ITEMS) || in_array("*.$ext", FM_ALLOWED_ITEMS)) {
        return true;
    }
    return false;
}
...
{
    global $lang, $sticky_navbar, $salir_url;
    ...
    <nav class="navbar navbar-expand-lg mb-4 main-nav <?php echo $isStickyNavBar ?>">
        <a href="<?php echo $salir_url ?>" class="regreso_boton_wrap">
            <span class="fas fa-arrow-circle-left fa-2x regreso_boton"></span>
            <span><p>SALIR</p></span>
        </a>
        <a href="#" class="logo_tutecho"><img src="../../objetos/logotipo2.svg" alt="Tu Techo.com" class="logo_img"></a>
...
<head>
    ...
    <title>TuTecho File Manager</title>
 ...
 */
function fm_show_header($moneda)
{
$sprites_ver = '20160315';
header("Content-Type: text/html; charset=utf-8");
header("Expires: Sat, 26 Jul 1997 05:00:00 GMT");
...
global $lang, $root_url, $sticky_navbar, $favicon_path, $modo_validacion;
...
<head>
    ...
    <?php endif; ?>
    <link rel="stylesheet" href="../../css/file_visualizer.css">
    <script>
        const moneda = "<?= $moneda ?>";
    </script>
    ...
    <?php
        if (isset($modo_validacion)) {
            echo "<script src=\"../../js/validar_file.js\"></script>";
        };
    ?>
...
if (isset($modo_validacion)) {
    ...
    <form id=\"formulario_validacion_file\" method=\"post\" action=\"" . htmlspecialchars($_SERVER['REQUEST_URI']) . "\">
        <input type=\"hidden\" name=\"accion_selected\" id=\"accion_selected\" value=\"\">

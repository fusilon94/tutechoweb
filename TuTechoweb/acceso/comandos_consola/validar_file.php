...
if (isset($_SESSION['usuario'])) {//si una SESSION a sido definida entonces dejar pasar, si no redirigir a login.php
  ...
  if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($tipo_file_selected == 'personal') {
        ...
        if (strpos($id_file, 'C') !== false) {
            $tabla = "borradores_casa";
          } else {
            if (strpos($id_file, 'D') !== false) {
              $tabla = "borradores_departamento";
            } else {
              if (strpos($id_file, 'L') !== false) {
                $tabla = "borradores_local";
              } else {
                if (strpos($id_file, 'T') !== false) {
                  $tabla = "borradores_terreno";
                };
              };
            };
          };

        $consulta_agente_destinatario =	$conexion->prepare("SELECT jefe_agencia_id FROM $tabla WHERE referencia = :referencia");
        ...
        if ($accion_selected == 'reclamo') {
            ...
            if ($agencia_asociada['nivel_acceso'] == 10) {
            ...
            if ($datos_precio['contrato_especial'] == 1) {
            ...
            $statement_activate_agente = $conexion->prepare(
              "UPDATE $tabla SET validacion_jefe_agencia = 1, fecha_validacion_jefe_agencia = :fecha_validacion_jefe_agencia, fotos360_solicitados = :fotos360_solicitados, vr_solicitado = :vr_solicitado, comision = :comision, agente_designado_id = :agente_designado_id WHERE referencia = :referencia");

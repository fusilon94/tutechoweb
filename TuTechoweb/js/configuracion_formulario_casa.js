...
// CODIGO VALIDACION FORMULARIO - al hacer click en VALIDAR ###################################
    ...
    $('#boton_validar_form').on('click', function () {
      ...
      if (value.length>=0 && (value.length !== reqlength)) {
          ...
          if ($('#superficie_inmueble').val() == '' || $('#superficie_terreno').val() == '' || $('#dormitorios').val() == '' || $('#parqueos').val() == '' || $('#precio').val() == '') {
            $('#panel_4 i.fa-circle').css('display', 'inline-block');
          };
          if ($('#pisos').val() == '' || $('#cocina').val() == '' || $('#wc').val() == '' || $('#tipo_ducha').val() == '' || $('#ventanas').val() == '' || $('#conexion_electrica').val() == '' || $('#cobertura').val() == '' || $('#internet').val() == '' || $('#tv_cable').val() == '' || $('#interior_estado').val() == '') { //Aun con el cambio de color en Atom, el codigo funciona muy bien
...
// ++++ OPENER VENTA INFO
        ...
        $( "#opener_pisos" ).click( function(event) {
        event.preventDefault();
        $( "#info_ventana_pisos" ).dialog( "open" );
        });

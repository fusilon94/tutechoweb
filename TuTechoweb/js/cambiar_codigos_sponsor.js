...
// SE DEFINE LA FUNCION QUE PERMITE TRAER LOS SPONSORS DE LA BASE DE DATOS #####################################
      ...
      function find_sponsors(barrio_param){
            ...
            $.ajax({
                type: "POST",
                url: "process-request-sponsors-with-codes.php",
              ...
              if (Object.entries(sponsors_array).length > 0) {//si no es un array vacio
                ...
                sponsors_array.forEach(function(sponsor){
                  if (sponsor['visibilidad'] == 'visible') {
                    $('.resultados_container').append("<div class='boton_borrador_mini_contenedor'><div id='" + sponsor['nombre'] + "' class='boton_borrador_formulario'><i class='fas fa-search' aria-hidden='true'></i><p><span class='nombre'>" + sponsor['label']  + "- </span><span>"  + sponsor['direccion'] + "</span></p></div><div class='boton_borrador_formulario_borrar' onclick='confirmacion(this)'><i class='fas fa-sync-alt' aria-hidden='true'></i></div><div class='boton_borrador_formulario_borrar_confirmar' onclick='confirmacion_borrar(this)'><p>CAMBIAR?</p></div></div>");
                  };
                });

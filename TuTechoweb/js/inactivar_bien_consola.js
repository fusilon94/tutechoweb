...
//CODIGO PARA MOSTRAR RESULTADOS SPONSOR SEGUN EL BARRIO ESCOGIDO ###############################################
        ...
        $("select.barrio").change(function(){
          ...
          if (barrioSelected !== '') { //
            ...
            $('.switch_sponsors').css('visibility', 'hidden');
            $('#switch_value').val('');
            $('#switch_reference_param').val("");
            $('#switch_barrio_param').val(barrioSelected);
...
// CODIGO PARA BUSQUEDA SEGUN REFERENCIA ############################################################
   ...
   $(".input_referencia_btn").on("click", function(){
     ...
     $("#ciudad").empty().prop('disabled', true).val('');
     $("#barrio").empty().prop('disabled', true).val('');
     $('#switch_reference_param').val(referencia_picked);
     $('#switch_barrio_param').val("");
      ...
      function find_bien_inmueble(barrio_param){
            $.ajax({
              ...
              if (Object.entries(bienes_array).length > 0) {//si no es un array vacio
                ...
                $('.resultados_container').append("<span class='label_resultadors_contenedor'>Bienes Inmuebles activos en: " + barrio_param + "</span>");

                bienes_array.forEach(function(bien){
                  if (bien['visibilidad'] == 'visible' && bien['inactivacion_autorizacion'] == 1) {
                    $('.resultados_container').append("<div class='boton_borrador_mini_contenedor'><div id='" + bien['referencia'] + "' name='" + bien['tipo_bien'] + "' class='boton_borrador_formulario'><i class='fas fa-search' aria-hidden='true'></i><p><span class='nombre'>" + bien['referencia'] + "</span></p></div><div class='boton_borrador_formulario_borrar' onclick='confirmacion(this)'><i class='fas fa-trash-alt' aria-hidden='true'></i></div><div class='boton_borrador_formulario_borrar_confirmar btn_inactivar' onclick='confirmacion_borrar(this)'><p>INACTIVAR?</p></div></div>");
                  };
                });
              ...
              if ($(".boton_borrador_mini_contenedor").length == 0) {
                $('.resultados_container').append("- NO EXISTEN BIENES INMUEBLES QUE MOSTRAR -");
              };
          ...
          function find_bien_inmueble_by_reference(reference_param){
            $.ajax({
              ...
              if (bienes_array.length > 0) {//si no es vacio
                ...
                $('#switch_value').val('activos');
                $('.resultados_container').append("<span class='label_resultadors_contenedor'>Bienes Inmuebles activos en: " + bienes_array[0]['location_tag'] + "</span>");

                  if (bienes_array[0]['visibilidad'] == 'visible' && bienes_array[0]['inactivacion_autorizacion'] == 1) {
                    $('.resultados_container').append("<div class='boton_borrador_mini_contenedor'><div id='" + bienes_array[0]['referencia'] + "' name='" + bienes_array[0]['tipo_bien'] + "' class='boton_borrador_formulario'><i class='fas fa-search' aria-hidden='true'></i><p><span class='nombre'>" + bienes_array[0]['referencia'] + "</span></p></div><div class='boton_borrador_formulario_borrar' onclick='confirmacion(this)'><i class='fas fa-trash-alt' aria-hidden='true'></i></div><div class='boton_borrador_formulario_borrar_confirmar btn_inactivar' onclick='confirmacion_borrar(this)'><p>INACTIVAR?</p></div></div>");
                  };
...
// ############################## SWITCH #######################################
      ...
      $('.switch_sponsors').on('click', '.switch', function(){
        ...
        var reference_param = $('#switch_reference_param').val();
        var barrio_param = $('#switch_barrio_param').val();
        var agencia_id = $("#agencia_id").val();
        ...
        if ($('#switch_value').val() == 'activos') {//si estamos en activos, mostrar los sponsors activos
          ...
          if (reference_param !== '') {
            ...
            $.ajax({
              ...
              var bienes_array = data;
                      ...
                      if (Object.entries(bienes_array).length > 0) {//si no es un array vacio
                        $('.resultados_container').append("<span class='label_resultadors_contenedor'>Bienes Inmuebles activos:</span>");
          ...
          if (reference_param !== '') {
            ...
            $.ajax({
              ...
              if (Object.entries(bienes_array).length > 0) {//si no es un array vacio
                $('.resultados_container').append("<span class='label_resultadors_contenedor'>Bienes Inmuebles inactivos:</span>");

                bienes_array.forEach(function(bien){
                  if (bien['visibilidad'] == 'no_visible' && bien['reactivacion_autorizacion'] == 1) {
                    $('.resultados_container').append("<div class='boton_borrador_mini_contenedor'><div id='" + bien['referencia'] + "' name='" + bien['tipo_bien'] + "' class='boton_borrador_formulario'><i class='fas fa-search' aria-hidden='true'></i><p><span class='nombre'>" + bien['referencia'] + "</span></p></div><div class='boton_borrador_formulario_borrar' onclick='confirmacion2(this)'><i class='fas fa-power-off' aria-hidden='true'></i></div><div class='boton_borrador_formulario_borrar_confirmar btn_reactivar' onclick='confirmacion_reactivar(this)'><p>REACTIVAR?</p></div></div>");
                  };
                });
...
// ################### CODIGO PARA MOSTRAR INFO DEL BIEN INMUEBLE #####################################
    ...
    $(".resultados_container").on("click", '.boton_borrador_formulario', function(){
      ...
      $.ajax({
          type: "POST",
          url: "process-request-form-info-bien_reactivar.php",

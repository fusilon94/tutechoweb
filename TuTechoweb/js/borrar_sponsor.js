...
// SE DEFINE LA FUNCION QUE PERMITE TRAER LOS SPONSORS DE LA BASE DE DATOS #####################################
      ...
      function find_sponsors(barrio_param){
            ...
            $.ajax({
                type: "POST",
                url: "process-request-sponsors-inactivar.php",
              ...
              if (Object.entries(sponsors_array).length > 0) {//si no es un array vacio
                ...
                sponsors_array.forEach(function(sponsor){
                  if (sponsor['visibilidad'] == 'visible' && sponsor['inactivacion_autorizacion'] == 1) {
      ...
      $('.switch_sponsors').on('click', '.switch', function(){
        ...
        if ($('#switch_value').val() == 'activos') {//si estamos en activos, mostrar los sponsors activos
          ...
          $.ajax({
              type: "POST",
              url: "process-request-sponsors-inactivar.php",
              data: { barrio_sent : barrio_sponsor },
              dataType: 'json'//SE ESPECIFICA QUE SE ESPERA DATA EN MODO JSON
          }).done(function(data){
            var sponsors_array = data;
          ...
          $.ajax({
            ...
            if (Object.entries(sponsors_array).length > 0) {//si no es un array vacio
              ...
              sponsors_array.forEach(function(sponsor){
                if (sponsor['visibilidad'] == 'no_visible' && sponsor['reactivacion_autorizacion'] == 1) {
                  $('.resultados_container').append("<div class='boton_borrador_mini_contenedor'><div id='" + sponsor['nombre'] + "' class='boton_borrador_formulario'><i class='fas fa-search' aria-hidden='true'></i><p><span class='nombre'>" + sponsor['label']  + "- </span><span>"  + sponsor['direccion'] + "</span></p></div><div class='boton_borrador_formulario_reactivar' onclick='confirmacion_reactivar(this)'><i class='fas fa-power-off' aria-hidden='true'></i></div><div class='boton_borrador_formulario_reactivar_confirmar' onclick='confirmacion_reactivar_validar(this)'><p>REACTIVAR?</p></div></div>");
                };
              });

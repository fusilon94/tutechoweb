...
$('.resultados_sponsors').on("click", ".boton_formulario_borrar_confirmar", function(){
  var referencia_autorizar = $(this).parent().find('.boton_borrador_formulario').attr('id');
  var tabla_autorizar = $(this).parent().find('.boton_borrador_formulario').attr('name');
  var linea_que_borrar = $(this).parent();
  $.ajax({
      type: "POST",
      url: "process-request-autorizar_reactivacion.php",
      data: { referencia_autorizar_sent : referencia_autorizar, tabla_autorizar_sent : tabla_autorizar },
  }).done(function(data){
    linea_que_borrar.remove();
...
// SE DEFINE LA FUNCION QUE PERMITE TRAER LOS SPONSORS DE LA BASE DE DATOS #####################################
      ...
      function find_bien_inmueble(barrio_param){
            $.ajax({
                type: "POST",
                url: "process-request-borrar-inmueble-resultados.php",

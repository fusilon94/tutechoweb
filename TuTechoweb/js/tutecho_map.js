...
//Se trae las coordenadas de los markers de Departamentos del Pais en carga inicial ################################
...
function armado_markers(bienes_inmuebles, call_type){
  ...
  $.each(bienes_inmuebles, function( departamento, inmuebles ) {//de cada departamento se ve su array de resultados
    ...
    $.each(inmuebles, function(index, info_inmueble) {//de cada resultado del array se busca colocar un puntero en el mapa
      if (info_inmueble['estado'] == 'En Venta') {
        if (info_inmueble['exclusivo'] == 1) {
            ...
            if (info_inmueble['pre_venta'] == 1) {// MARKER: blue + PV
              ...
              var marker_individual = new L.marker([(parseFloat(info_inmueble['mapa_coordenada_lat'])+0.000350), (parseFloat(info_inmueble['mapa_coordenada_lng'])+0.000350)], {icon: marcador_tutecho, marker_id: info_inmueble['referencia']});

              marker_individual.addTo(mymap).bindPopup(popup_string, {closeButton: false, className: "popup_inmueble"}).on('mouseover', function (e) {this.openPopup(); onMarkerHoverIn(this);}).on('mouseout', function (e) {this.closePopup(); onMarkerHoverOut(this);}).bindTooltip("PV", {permanent: true, opacity: 0.9, className: 'tooltip_inmueble', direction: 'center', offset: [2.8,64]})._icon.classList.add("marcador_inmueble");

              new L.circle([(parseFloat(info_inmueble['mapa_coordenada_lat'])+0.000350), (parseFloat(info_inmueble['mapa_coordenada_lng'])+0.000350)], {
                color: 'rgb(51, 136, 255, 0)',//sin color
                fillColor: 'rgb(51, 136, 255)',
                fillOpacity: 0.3,
                radius: 200,
                className: 'circle_map circle_' + info_inmueble['referencia'].replace("#", "")
              }).addTo(mymap);
              ...
              marker_individual.addTo(mymap).bindPopup(popup_string, {closeButton: false, className: "popup_inmueble"}).on('mouseover', function (e) {this.openPopup(); onMarkerHoverIn(this);}).on('mouseout', function (e) {this.closePopup(); onMarkerHoverOut(this);})._icon.classList.add("marcador_inmueble");
      ...
      if (info_inmueble['estado'] == 'En Alquiler') {
        if (info_inmueble['exclusivo'] == 1) {
            ...
            if (info_inmueble['anticretico'] == 1) {// MARKER: blue + A
              ...
              var popup_string =  popup_marker_inmueble(info_inmueble);
              var marker_individual = new L.marker([(parseFloat(info_inmueble['mapa_coordenada_lat'])+0.000350), (parseFloat(info_inmueble['mapa_coordenada_lng'])+0.000350)], {icon: marcador_tutecho, marker_id: info_inmueble['referencia']});
              marker_individual.addTo(mymap).bindPopup(popup_string, {closeButton: false, className: "popup_inmueble"}).on('mouseover', function (e) {this.openPopup(); onMarkerHoverIn(this);}).on('mouseout', function (e) {this.closePopup(); onMarkerHoverOut(this);}).bindTooltip("A", {permanent: true, opacity: 0.9, className: 'tooltip_inmueble', direction: 'center', offset: [2.8,64]})._icon.classList.add("marcador_inmueble");
      ...
      oms.addMarker(marker_individual);//se agrega del marker al overlapping map spiderfy
...
function popup_marker_inmueble_aprox_text(exclusivo){
    if (exclusivo == 0) {
        return "<p class=\"text_aprox\">- NO EXCLUSIVO -</br>Ubicación Aproximada</p>"
    }else{
        return "<p class=\"text_aprox\">- EXCLUSIVO -</p>"
    };
};

function popup_marker_inmueble_tag(info_inmueble){
    if (info_inmueble['estado'] == 'En Venta' && info_inmueble['pre_venta'] == 1) {
        return "<span class=\"tag_extra\">Pre-Venta</span>"
    }else if (info_inmueble['estado'] == 'En Alquiler' && info_inmueble['anticretico'] == 1) {
        return "<span class=\"tag_extra\">Anticretico</span>"
    }else{
      return "";
    };
};
...
function popup_marker_inmueble(info_inmueble){
  ...
  if (info_inmueble['tipo_bien'] == 'terreno') {
    ...
    if (info_inmueble['superficie_terreno_medida'] == 'hect') {
      var string_made = popup_marker_inmueble_tag(info_inmueble) + "<div class='marker_popup_container'><span class='popup_entire_group'><span>" + info_inmueble['precio'].replace(/\B(?=(\d{3})+(?!\d))/g, " ") + " $us</span><span>" + (info_inmueble['superficie_terreno']/10000) + " Hect</sup></span></span></div>" + popup_marker_inmueble_aprox_text(info_inmueble['exclusivo']);
    }else {//entonces es en m2
      var string_made = popup_marker_inmueble_tag(info_inmueble) + "<div class='marker_popup_container'><span class='popup_entire_group'><span>" + info_inmueble['precio'].replace(/\B(?=(\d{3})+(?!\d))/g, " ") + " $us</span><span>" + info_inmueble['superficie_terreno'] + " m<sup>2</sup></sup></span></span></div>" + popup_marker_inmueble_aprox_text(info_inmueble['exclusivo']);
    };

  }else if (info_inmueble['tipo_bien'] == 'local') {//entonces es local

    var string_made = popup_marker_inmueble_tag(info_inmueble) + "<div class='marker_popup_container'><span class='popup_half_group'><span>" + info_inmueble['precio'].replace(/\B(?=(\d{3})+(?!\d))/g, " ") + " $us</span><span>" + info_inmueble['superficie_inmueble'] + " m<sup>2</sup></span></span><span class='popup_half_group'><span><i class='fa fa-car'></i> x" + info_inmueble['parqueos'] + "</span></span></div>" + popup_marker_inmueble_aprox_text(info_inmueble['exclusivo']);
...
// SE DEFINE LA FUNCION QUE MANDA LOS DATOS DE BUSQUEDA AVANZADA AL PROCESS REQUEST ########################################
  ...
  function onMarkerHoverIn(e){
    var circle_class = '.circle_map.circle_' + e.options.marker_id.replace("#", "");//porque no se pueden usar # en classes
    $(circle_class).css('opacity', 1);
  };

  function onMarkerHoverOut(e){
    var circle_class = '.circle_map.circle_' + e.options.marker_id.replace("#", "");//porque no se pueden usar # en classes
    $(circle_class).css('opacity', 0);
  };
...
// CODIGO PARA TRAER LA FICHA BIEN DESPUES DE HACER CLICK EN UN MARKER BIEN INMUEBLE ####################################
  function onMarkerInmuebleClick(e){
      ...
      $.ajax({
            type: "POST",
            url: "process-request-popup_ficha_bien_tutecho_map.php",
...
// CODIGO PARA CARGAR IMNUEBLES EN VENTA O RENTA SEGUN HASH ################################################

let hashVal = window.location.hash.substr(1);
if (hashVal == 'venta') {
    var venta_btn = "radio_estado-1.1";//RESTABLECE EL CHECKBOX DOMITORIOS
    $(document.getElementById(venta_btn).previousElementSibling).click();
};
if (hashVal == 'renta') {
    var renta_btn = "radio_estado-1.2";//RESTABLECE EL CHECKBOX DOMITORIOS
    $(document.getElementById(renta_btn).previousElementSibling).click();
};

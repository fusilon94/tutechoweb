...
// CODIGO VALIDACION FORMULARIO - al hacer click en VALIDAR ###################################
    ...
    $('#boton_validar_form').on('click', function () {
      ...
      if (value.length>=0 && (value.length !== reqlength)) {
          ...
          if ($('#dormitorios').val() == '' || $('#parqueos').val() == '' || $('#precio').val() == '' || $('#piso').val() == '') {
            $('#panel_4 i.fa-circle').css('display', 'inline-block');
          };
          if ($('#tipo_departamento').val() == '' || $('#cocina').val() == '' || $('#wc').val() == '' || $('#tipo_ducha').val() == '' || $('#ventanas').val() == '' || $('#conexion_electrica').val() == '' || $('#cobertura').val() == '' || $('#internet').val() == '' || $('#tv_cable').val() == '' || $('#interior_estado').val() == '') { //Aun con el cambio de color en Atom, el codigo funciona muy bien

...
$('.resultados_sponsors').on("click", ".boton_formulario_borrar_confirmar", function(){
  var referencia_que_borrar = $(this).parent().find('.boton_borrador_formulario').attr('id');
  var linea_que_borrar = $(this).parent();
  $.ajax({
      type: "POST",
      url: "process-request-borrar_inmueble.php",
      data: { referencia : referencia_que_borrar },
  }).done(function(data){
    linea_que_borrar.remove();
    $("#mensaje_exito").val(data);
    $("#formulario_exito").submit();
  });
});
